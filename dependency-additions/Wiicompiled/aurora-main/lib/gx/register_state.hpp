#pragma once
// GX register state and shader inputs shared by GPU backends. Resource handles
// and WebGPU pipeline/cache objects remain in GXState (gx.hpp).
#include "../internal.hpp"
#include <dolphin/gx.h>
#include <aurora/math.hpp>
#include "../gfx/render_types.hpp"
...
struct AlphaCompare {
  ...
  // ALWAYS XOR ALWAYS rejects every fragment; it cannot skip alpha testing.
  explicit operator bool() const { return comp0 != GX_ALWAYS || comp1 != GX_ALWAYS || op == GX_AOP_XOR; }
};
...
GXRegisterState& register_state() noexcept;

inline void mark_pipeline_state_dirty() noexcept {
  auto& state = register_state();
  state.stateDirty = true;
  state.pipelineStateGeneration = next_gx_state_epoch();
}
...
// Backend mapping is resolved when decoding updates viewport or scissor.
